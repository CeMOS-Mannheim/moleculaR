...
simulatespp <- function(lambdaHotspot, lambdaBackground,
         ...
         bgspp <- analytePointPattern(x = bgspp$x, y = bgspp$y, win = bgspp$window,
                                                 intensity = bgspp$marks, mzVals = "simulated")

         hotspotspp <- analytePointPattern(x = hotspotspp$x, y = hotspotspp$y, win = hotspotspp$window,
                                                      intensity = hotspotspp$marks, mzVals = "simulated")
         ...
         # superimpose
         gtspp <- superimposeAnalytes(list(bgspp, hotspotspp))

         if(plot){
                  plotAnalyte(gtspp, size = 1, show.window = FALSE,
